...
async Init1(stub ,args,thisClass) {
 ...
 let storedData = 0;
    await stub.putState('storedData', Buffer.from(storedData.toString()));

 } 
async set(stub, args, thisClass) {
if (args.length != 2 ){
...
let temp0 = await stub.getState('storedData');
      let storedData = parseFloat(temp0);
