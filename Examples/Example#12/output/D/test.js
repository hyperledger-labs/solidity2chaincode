...
async Init1(stub ,args,thisClass) {
 ...
 let data = 0;
    await stub.putState('data', Buffer.from(data.toString()));

 } 
async f(stub, args, thisClass) {
if (args.length != 2 ){
...
let b = 0;
let temp0 = await stub.getState('data');
      let data = parseFloat(temp0);
...
async setData(stub, args, thisClass) {
if (args.length != 2 ){
            throw new Error('Incorrect number of arguments. Expecting 2');
          }
...
async getData(stub, args, thisClass) {
if (args.length != 1 ){
...
async compute(stub, args, thisClass) {
if (args.length != 3 ){
...
async readData(stub, args, thisClass) {
if (args.length != 1 ){
            throw new Error('Incorrect number of arguments. Expecting 1');
          }
  ...
  let c=new C();
        await c.Constructor(stub);
  let local=c.f(stub,[ msg.value,7],thisClass);
        
  c.setData(stub,[ msg.value,3],thisClass);
  local=c.getData(stub,[ msg.value],thisClass);
  local=c.compute(stub,[ msg.value,3,5],thisClass);
