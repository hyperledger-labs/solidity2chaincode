...
async Init1(stub ,args,thisClass) {
 ...
 let choice = 0;
    await stub.putState('choice', Buffer.from(choice.toString()));
 let defaultChoice =  undefined /* use index of Enum Type */;
    await stub.putState('defaultChoice', Buffer.from(defaultChoice.toString()));

 } 
async setGoStraight(stub, args, thisClass) {
if (args.length != 1 ){
...
let temp0 = await stub.getState('choice');
      let choice = temp0.toString();
let temp1 = await stub.getState('defaultChoice');
      let defaultChoice = temp1.toString();

  choice= undefined /* use index of Enum Type */;
  
  await stub.putState('choice', Buffer.from(choice.toString()));

}
async getChoice(stub, args, thisClass) {
if (args.length != 1 ){
            throw new Error('Incorrect number of arguments. Expecting 1');
          }
...
async getDefaultChoice(stub, args, thisClass) {
if (args.length != 1 ){
            throw new Error('Incorrect number of arguments. Expecting 1');
          }
